...
class Model(object):
    ...
    def forward_net(self, x, drop_prob, reuse=False):
        ...
        with tf.compat.v1.variable_scope('net_encoder', reuse=reuse) as scope:
            cur_input = x
            ...
            for i in range(self.num_net_layers):
                ...
                if self.is_init:
                    cur_input = tf.compat.v1.layers.dense(cur_input, units=struct[i],
                                                kernel_initializer=tf.constant_initializer(self.W_init[name]),
                                                bias_initializer=tf.constant_initializer(self.b_init[name]))
                else:
                    cur_input = tf.compat.v1.layers.dense(cur_input, units=struct[i], kernel_initializer=w_init())
                if i < self.num_net_layers - 1:
                    cur_input = lrelu(cur_input)
                    cur_input = tf.compat.v1.layers.dropout(cur_input, drop_prob)
                print(cur_input.get_shape())
            ...
            for i in range(self.num_net_layers - 1):
                ...
                if self.is_init:
                    cur_input = tf.compat.v1.layers.dense(cur_input, units=struct[i + 1],
                                                kernel_initializer=tf.constant_initializer(self.W_init[name]),
                                                bias_initializer=tf.constant_initializer(self.b_init[name]))
                else:
                    cur_input = tf.compat.v1.layers.dense(cur_input, units=struct[i + 1], kernel_initializer=w_init())
                cur_input = lrelu(cur_input)
                cur_input = tf.compat.v1.layers.dropout(cur_input, drop_prob)
                print(cur_input.get_shape())
            ...
            if self.is_init:
                cur_input = tf.compat.v1.layers.dense(cur_input, units=self.net_input_dim,
                                            kernel_initializer=tf.constant_initializer(self.W_init[name]),
                                            bias_initializer=tf.constant_initializer(self.b_init[name]))
            else:
                cur_input = tf.compat.v1.layers.dense(cur_input, units=self.net_input_dim, kernel_initializer=w_init())
    ...
    def forward_att(self, x, drop_prob, reuse=False):
        ...
        with tf.compat.v1.variable_scope('att_encoder', reuse=reuse) as scope:
            cur_input = x
            print(cur_input.get_shape())
            ...
            if self.is_init:
                cur_input = tf.compat.v1.layers.dense(cur_input, units=self.att_input_dim,
                                            kernel_initializer=tf.constant_initializer(self.W_init[name]),
                                            bias_initializer=tf.constant_initializer(self.b_init[name]))
            else:
                cur_input = tf.compat.v1.layers.dense(cur_input, units=self.att_input_dim, kernel_initializer=w_init())
            # cur_input = tf.nn.sigmoid(cur_input)
            x_recon = cur_input
            print(cur_input.get_shape())
